!(function (e) {
  ...
  var g = function (k, j) {
    ...
    this.language =
      this.language in f ? this.language : this.language.split("-")[0];
    this.language = this.language in f ? this.language : "en";
    this.isRTL = f[this.language].rtlTime || false;
    this.formatType =
      j.formatType || this.element.data("format-type") || "standard";
    this.format = d.parseFormat(
      j.format ||
        this.element.data("date-format") ||
        f[this.language].format ||
        d.getDefaultFormat(this.formatType, "input"),
      this.formatType
    );
    this.isInline = false;
    ...
    this.componentReset = this.element.is(".date")
      ? this.element
          ...
          .parent()
      : false;
    if ("componentIcon" in j) {
      this.component = this.element
        .find(".input-group-addon " + j.componentIcon)
        .parent();
    }
    ...
    this.linkFormat = d.parseFormat(
      j.linkFormat ||
        this.element.data("link-format") ||
        d.getDefaultFormat(this.formatType, "link"),
      this.formatType
    );
    ...
    if ("navIcons" in j) {
      if (j.navIcons.rightIcon && j.navIcons.rightIcon) {
        this.icons = {
          leftArrow: j.navIcons.leftIcon,
          rightArrow: j.navIcons.rightIcon,
        };
        if (j.navIcons.iconType) {
          this.icontype = j.navIcons.iconType;
        } else {
          this.icontype = "icon";
        }
      }
    ...
    if (this.isRTL) {
      ...
      var i = ".prev span, .next span";
      this.picker
        .find(i)
        .toggleClass(this.icons.leftArrow + " " + this.icons.rightArrow);
    }
    e(document).on("mousedown touchstart", function (n) {
  ...
  g.prototype = {
    ...
    place: function () {
      ...
      l = l - j.top;
      k = k - j.left;
      if (l + this.picker.outerHeight() > e(window).height()) {
        l = e(window).height() - this.picker.outerHeight();
      }
      if (l < 0) {
        l = 0;
      }
    ...
    fill: function () {
      ...
      this.picker
        ...
        .text(f[this.language].months[I] + " " + u);
      if (this.formatViewType == "time") {
        ...
        this.picker
          ...
          .text(n + " " + f[this.language].months[I] + " " + u);
        this.picker
      ...
      while (K.valueOf() < j) {
        ...
        if (
          K.valueOf() + 86400000 <= this.startDate ||
          K.valueOf() > this.endDate ||
          e.inArray(K.getUTCDay(), this.daysOfWeekDisabled) !== -1
        ) {
          v += " disabled";
        }
      ...
      for (var B = 0; B < 24; B++) {
        ...
        v = "";
        if (
          x.valueOf() + 3600000 <= this.startDate ||
        ...
        if (this.showMeridian && f[this.language].meridiem.length == 2) {
          F =
            B < 12
              ? f[this.language].meridiem[0]
              : f[this.language].meridiem[1];
          if (F != t) {
            if (t != "") {
              s.push("</fieldset>");
            }
            s.push(
              '<fieldset class="hour"><legend>' + F.toUpperCase() + "</legend>"
            );
          }
          t = F;
          w = B % 12 ? B % 12 : 12;
      ...
      for (var B = 0; B < 60; B += this.minuteStep) {
        ...
        if (x.valueOf() < this.startDate || x.valueOf() > this.endDate) {
          ...
          if (
            Math.floor(y / this.minuteStep) == Math.floor(B / this.minuteStep)
          ) {
            v += " active";
          }
        }
    ...
    showMode: function (i) {
      ...
      this.picker
        .find(">div")
        .hide()
        .filter(".datetimepicker-" + d.modes[this.viewMode].clsName)
        .css("display", "block");
      this.updateNavArrows();
    },
    reset: function (i) {
  ...
  e.fn.datetimepicker = function (k) {
    ...
    this.each(function () {
      ...
      if (!m) {
        n.data(
          "datetimepicker",
          (m = new g(this, e.extend({}, e.fn.datetimepicker.defaults, l)))
        );
      }
    ...
    formatDate: function (j, o, q, m) {
      ...
      if (m == "standard") {
        p = {
          ...
          p:
            f[q].meridiem.length == 2
              ? f[q].meridiem[j.getUTCHours() < 12 ? 0 : 1]
              : "",
          h: j.getUTCHours(),
          i: j.getUTCMinutes(),
          s: j.getUTCSeconds(),
        ...
        if (m == "php") {
          p = {
            ...
            S:
              j.getUTCDate() % 10 <= f[q].suffix.length
                ? f[q].suffix[(j.getUTCDate() % 10) - 1]
                : "",
            a:
              f[q].meridiem.length == 2
                ? f[q].meridiem[j.getUTCHours() < 12 ? 0 : 1]
